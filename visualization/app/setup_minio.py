...
def save_to_dotenv(access_key, secret_key):
    ...
    try:
        ...
        console.print(
            "[bold green]>>> Success: Credentials saved to .env file.[/bold green]"
        )
    except Exception as e:
...
def load_minio_client() -> Minio:
    ...
    try:
        ...
        if access_key and secret_key:
            ...
            if client:
                console.print(
                    "[bold green]>>> Successfully connected to MinIO![/bold green]\n"
                )
                return client

        # Attempt user provided input
        console.print(
            "[bold yellow][!] minio credentials (minio_access_key/secret_key) not found.[/bold yellow]"
        )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            console.print(
                " [cyan]*[/cyan] Provide the path to a valid [green].env[/green] file"
            )
            console.print(
                " [cyan]*[/cyan] OR type your MinIO [blue]Access Key[/blue] directly"
            )

            user_input = Prompt.ask(
                "[bold magenta]>> Enter file path OR Access Key[/bold magenta]"
            ).strip()
            ...
            if file_path.is_file():
                console.print(
                    f"[bold yellow][#] File detected.[/bold yellow] Loading from '[cyan]{file_path}[/cyan]'..."
                )
                ...
                secret_input = Prompt.ask(
                    "[bold magenta]>> Enter MinIO Secret Key[/bold magenta]",
                    password=True,
                ).strip()
            ...
            if access_key and secret_key:
                ...
                if client:
                    ...
                    if stdin_input and Confirm.ask(
                        "[bold yellow]?? Save these to local .env?[/bold yellow]",
                        default=False,
                    ):
                ...
                console.print(
                    "[bold red][!] No keys found in provided input.[/bold red]"
                )
...
def sync_verify_and_download(
    client: Minio, bucket: str, obj_name: str, file_path: Path, progress: Progress
) -> None:
    ...
    if file_path.exists() and hash_file_path.exists():
        ...
        if local_cached_hash == remote_etag:
            progress.update(
                task_id,
                description=f"[green]>>> {file_path.name} (Cached)[/green]",
                completed=stat.size,
                total=stat.size,
            )
            return
    ...
    progress.update(
        task_id,
        description=f"[yellow]Downloading {file_path.name}...",
        total=stat.size,
        completed=0,
    )
    ...
    progress.update(
        task_id, description=f"[bold green]>>> {file_path.name} (Complete)[/bold green]"
    )


async def ensure_files_downloaded(
    client: Minio, bucket: str, cache_dir: Path, required_files: dict[str, str]
):
