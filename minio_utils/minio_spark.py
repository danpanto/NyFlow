...
class MinioSparkClient:
    ...
    def __init__(
        ...
        self._spark_builder = (
            self._spark_builder.config("spark.jars", local_jars)
        ...
        self._spark_builder = (
            self._spark_builder.config("spark.hadoop.fs.s3a.endpoint", endpoint)
        ...
        self._spark_builder = (
            self._spark_builder.config("spark.hadoop.fs.s3a.path.style.access", "true")
            .config(
        ...
        self._spark_builder = (
            self._spark_builder.config("spark.storage.memoryFraction", "0.1")
    ...
    def __check_session(self):
        if not self._connected:
            raise RuntimeError(
                "Spark session is not running. Use connect() to initiate it"
            )
    ...
    def __write_file(
        self, _writer: DataFrameWriter, file_format: str, path: str, **options
    ):
        self.__check_session()
    ...
    def upload_file(self, local_path, minio_path):
        ...
        self._minio.fput_object(
            bucket_name=self._bucket, object_name=res, file_path=local_path
        )
