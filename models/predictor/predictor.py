...
class TaxiPredictor:
    def __init__(self, base_path="final_models"):
        ...
        self.scaler_espacial = self._load_joblib(
            self.darts_path / "scaler_espacial.pkl"
        )

        # Pipelines estándar usados en el entrenamiento
        self.pipeline_log = Pipeline(
            [InvertibleMapper(fn=np.log1p, inverse_fn=np.expm1)]
        )
    ...
    def _load_tide(self, subpath):
        ...
        try:
            ...
            print(
                f"⚠️ Error cargando modelo {nombre_modelo} en {work_dir_absoluto}: {e}"
            )
            return None
    ...
    def _preprocesar_input(self, df, value_col):
        ...
        estaticas = (
            list(self.encoder_vendor.get_feature_names_out(["VendorID"])) + cols_geo
        )
    ...
    def predecir_demanda(self, df_historia, df_covariables_futuras, horizonte=24):
        ...
        ts_past = TimeSeries.from_dataframe(
            df_historia, time_col="timestamp", value_cols=["avg_distance", "avg_amount"]
        )
