...
fechas_futuras = pd.date_range(
    start=fecha_actual + pd.Timedelta(hours=1), periods=24, freq="h"
)
...
if resultados_prediccion:
    ...
    df_predicciones_finales["pred_demand"] = np.clip(
        np.round(df_predicciones_finales["pred_demand"]), 0, None
    )
