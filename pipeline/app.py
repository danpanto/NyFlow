...
from textual.widgets import Footer, ContentSwitcher, Tab, Label
...
class Pipeline(App):
    ...
    def notify_and_log(
        self, message: str, title: str = "", status: str = "INFO", markup: bool = True
    ):
    ...
    def run_dl_pipeline(self):
        ...
        from data_extraction.download import get_lazy_frame, save_lazy_frame
        ...
        self.call_from_thread(
            lambda: [
                setattr(w, "disabled", True) for w in self.query("#dialog, #dialog2")
            ]
        )
        ...
        if dl_mode != "None" or len(vendors) == 0:
            for group in product(dates, vendors):
                ...
                self.notify_and_log(message="Please wait...", title="Downloading data")
                ...
                if transf:
                    self.notify_and_log(
                        message="Please wait...", title="Transforming columns"
                    )
                ...
                self.notify_and_log(
                    message="Please wait...", title="Saving data to file"
                )
                save_lazy_frame(
                    lf,
                    date[0],
                    date[1],
                    group[1],
                    self._client if dl_location != "Local" else None,
                )
                self.notify_and_log(
        ...
        self.call_from_thread(
            lambda: [
                setattr(w, "disabled", False) for w in self.query("#dialog, #dialog2")
            ]
        )
    ...
    def run_prep_pipeline(self):
        ...
        if prep_model and len(files) > 1 and not merge:
            self.notify(
                "Multiple files selected for aggregation, \
        ...
        if del_outliers:
            self.notify_and_log(message="Please wait...", title="Removing outliers")
        ...
        if merge:
            self.notify_and_log(message="Please wait...", title="Merging data")
        ...
        if prep_model:
            self.notify_and_log(
                message="Please wait...", title="Preparing data for model"
            )
    ...
    def compose(self) -> ComposeResult:
        yield Tabs(
            *[Tab(val, id=f"nav-tab-{i + 1}") for i, val in enumerate(self.TAB_LIST)]
        )
                        ...
                        with Vertical(id="dialog"):
                            ...
                            with Vertical(id="vendors-collapsable"):
                                for vendor in self.vendors:
                                    short_name = vendor.split(sep=" ", maxsplit=1)[
                                        0
                                    ].lower()
                            ...
                            with Vertical(id="date-collapsable"):
                                with Horizontal(
                                    classes="optbox_sub1-row middle-button"
                                ):
                                    yield Button(
                                        "Select Dates",
                            ...
                            with Vertical(classes="down-right"):
                                yield Button(
                                    "Download",
                                    action=self.run_dl_pipeline,
                                    classes="focuseable",
                                )
                                yield Button(
                                    "Exit", action=self._exit_app, classes="focuseable"
                                )
