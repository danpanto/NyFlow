...
nyc_schema_optimization_labels: dict[str, list[str]] = {
    "int8": ["VendorID", "RatecodeID", "passenger_count", "payment_type", "trip_type"],
...
def optimize_dataframe(
    ...
    transformations: dict[
        str, TransformationFunc
    ] = nyc_schema_optimization_transformations,
) -> AnyDataframe:
    """
    ...
    for category, cols in labels.items():
        ...
        if transform_fn is None:
            print(
                f"ERROR: Couldn't get transformation function for category '{category}'"
            )
            continue
...
def compare_polars_dfs(df_raw: pl.DataFrame, df_opt: pl.DataFrame):
    ...
    print(
        f"{'Metric':<20} | {'Raw DataFrame':<15} | {'Optimized':<15} | {'Improvement'}"
    )
    print("-" * 75)
    ...
    print(
        f"{'RAM Usage':<20} | {to_mb(mem_raw):>11.2f} MB | {to_mb(mem_opt):>11.2f} MB | {ram_gain:>9.1f}%"
    )

    # Storage Comparison
    storage_gain = (1 - (storage_opt / storage_raw)) * 100
    print(
        f"{'Est. Parquet Size':<20} | {to_mb(storage_raw):>11.2f} MB | {to_mb(storage_opt):>11.2f} MB | {storage_gain:>9.1f}%"
    )
